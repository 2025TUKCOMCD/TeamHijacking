...
public class RealtimeDTO {
    ...
    private int transportLocalID;       // 대중교통 : 정류장/역 ID (stid, lineId)
    private int stationId;              // 버스 : 노선 ID (busRouteId)
    private int startOrd;               // 버스 : 위치 정보 첫번째 순서
    private int endOrd;                 // 버스 : 위치 정보 마지막 순서
    private int trainNo;                // 지하철 : 기차 번호
    private String StartstationName;    // 지하철 : 지하철 역 이름
    private String SecondstationName;   // 지하철 : 두번째 지하철 역 이름
    private String EndstationName;      // 지하철 : 지하철 역 이름
